...
def save_to_excel(user_id, data, filename="onboarding_data.xlsx"):
    try:
        ...
        pdf_display = os.path.basename(pdf_path) if pdf_path else 'No file uploaded'
        ...
        new_data = {
            ...
            'PDF Document': pdf_display,  # Only show filename in the cell
            'PDF Path': pdf_path if pdf_path else '',  # Store full path for hyperlink
        ...
        if worksheet.max_row > 1:  # Check if there are data rows
            ...
            if 'PDF Path' in df.columns and 'PDF Document' in df.columns:
                pdf_path_col = df.columns.get_loc('PDF Path') + 1  # +1 because Excel is 1-based
                pdf_doc_col = df.columns.get_loc('PDF Document') + 1
                
                for row in range(2, worksheet.max_row + 1):  # Start from row 2 (after header)
                    pdf_path = worksheet.cell(row=row, column=pdf_path_col).value
                    if pdf_path and os.path.exists(pdf_path):
                        # Create a hyperlink to the PDF file
                        worksheet.cell(row=row, column=pdf_doc_col).hyperlink = pdf_path
                        worksheet.cell(row=row, column=pdf_doc_col).style = 'Hyperlink'
            
            # Hide the PDF Path column
            if 'PDF Path' in df.columns:
                pdf_path_col = df.columns.get_loc('PDF Path') + 1
                worksheet.column_dimensions[get_column_letter(pdf_path_col)].hidden = True
            ...
            if worksheet.max_row > 1:  # Only add filter if there are data rows
                worksheet.auto_filter.ref = f"A1:{get_column_letter(worksheet.max_column-1)}{worksheet.max_row}"  # -1 because we'll hide the PDF Path column
...
async def ask_next_question(ctx, user_id):
    ...
    for field, question in questions:
        if field not in user_data[user_id]:
            # Don't send message for PDF upload, it will be handled in on_message
            if field != 'pdf_upload':
